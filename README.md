...
Examples and utilities for the Advanced C++ course for the GUEIM association, in the Universidad Complutense de Madrid.
...
### GCC 5.2-5.3
Should already be available in any Linux distro. In the case of Ubunty, you may add the testing PPAs (`ubuntu-toolchain-r-test`) to install it.
You may see the installation process for Debian in the Travis CI build.

It is available for Windows trough the [nuwen MinGW distro](http://nuwen.net/mingw.html).

In case it is not available (in Cygwin, for example), you may try nd compile GCC by hand. It's really simple:
...
*Besides, you must take into accout that GTest and GMock are compiled for a specific configuration at compile time (that is, the `${CMAKE_BUILD_TYPE}` varuabke), so make sure that you have configured the project in the corresponding mode before launching the test, since it won't link otherwise.*

### Reflection

The reflection engine demonstrated in the course utilizes a Python script to analyze and generate the exta code necessary for refletion. In addition, said script utilizes the libclang API to analyze source code.
...
Besides, it is necessary to install `pip`, the Python dependency manager. The typical method is to download the script 
Además es necesario instalar `pip`, the Python dependency manager. The typical method is to download and execute the script [`get-pip.py`](https://bootstrap.pypa.io/get-pip.py):
...
2. **Install libclang:**
 ...
 - Linux: The name of the package may change from one distro to another. In Debian, for example, the one being used in the Travis builds is `libclang1-[VERSION CLANG]`. It is usually included as a package in the corresponding Clang.
...
Is a small repo where I have been uploading utilities to help using CMake. At the moment, it contains:
 ...
 ```
  ...
  creates an executable `example_patata` from the file `examples/patata.cpp`. 
 ...
 - [`cmakepp.cmake`](https://github.com/toeb/cmakepp): A framework that extendds CMake with lots of utilities. Although it's complex and lacks documentation, the creator usually answers questions in detail. *I have copied the compiled version of cmakepp so that I can modify some things, that's why it's not a submodule.*
 - `custom_properties.cmake`: Both the targets and the sources of a CMake project have a series of properties associated to them, that we can consult, modify, etc. However, the property set is fixed, we cannot add our own. This module implements a series of functions similar to the CMake property interface to defin our own properties for files and targets.

## Licence

The contents here described are published under the MIT Licence
